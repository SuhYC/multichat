...
void* recv_msg(void* s) {
    ...
    while (true) {
        ...
        if (str_len == -1) { // 서버 및 네트워크 문제
            cout << "disconnected" << endl;
            return NULL;
        }

        buf[str_len] = 0; // 쓰기 편하게 string으로 만들기
        msg = std::string(buf);
        ...
        if (chatcode.empty()) { // 참여중인 채팅방이 없는 경우
            ...
            else if(!strcmp(msg.c_str(),"fail")) { // 채팅방 코드 틀림
...
void* send_msg(void* s) {
    ...
    while (true) {
        ...
        if (!strcmp(msg.c_str(), "q") || !strcmp(msg.c_str(), "Q")) { // 나가기 입력 (채팅방 -> 로비 || 로비 -> 접속종료)
            mu.lock();
            if (chatcode.empty()) { // (로비 -> 접속종료)
                mu.unlock();
                closesocket(sock);
                exit(0);
            }
            else { // (채팅방 -> 로비), 채팅방 참여와는 달리 서버의 응답을 기다릴 필요가 없다.
                chatcode = "";
                system("cls");
