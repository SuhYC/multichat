...
- 1Send : 추가로 너무 많은 locked page가 발생하지 않도록 하는 역할을 한다고 한다. (메시지 송신 요청마다 추가 할당되므로 그 크기가 무한정 늘어나는 것을 어느정도 막을 수 있다.)
...
## 6. STL - std::queue
- std::queue<T>가 생각보다 느려서 queue의 기본 구성만 가지고 있는 myQueue클래스 추가. PacketData*만 담을 것이기 때문에 Non-template class로 구성.
# 문서
### 유즈케이스 다이어그램
