...
# 새로 얻은 경험
## 1. IOCP
- 기존 Blocking Socket 버전의 경우 1개의 클라이언트에 연결할 때마다 서버는 1개의 쓰레드를 생성해야 했다. 이 경우 연결된 클라이언트가 많아지면 쓰레드의 수가 많아지며 이는 Context Switching 비용이 증가하는 결과로 이어진다.
...
- 1Send : 비동기 입출력의 경우 요청한 순서대로 처리되지 않을 수 있다. (TCP의 Segment 순서보장과는 다르다. 하나의 메시지 내에서 분할이 이루어진건 순서가 보장된다.) 이를 방지하기 위해 각 연결마다 한번의 송신이 완료되면 다음 데이터를 송신하도록 한다.
